...
const App = {
  ...
  bindEvents() {
    ...
    document
      ...
      .addEventListener("change", (e) => this.toggleMode(e.target.checked));
    ...
    document
      ...
      .addEventListener("click", () => this.startImport());

    // Export generated files
    document
      .getElementById("export-generated-btn")
      .addEventListener("click", () => this.exportGenerated());
    ...
    document
      ...
      .addEventListener("click", () => this.startGenerate());

    // Send
    document
      .getElementById("send-btn")
      .addEventListener("click", () => this.startSend());

    // Send mode toggle (with/without generate)
    document
      .getElementById("send-mode-toggle")
      .addEventListener("change", (e) => this.toggleSendMode(e.target.checked));
    ...
    // Direct send
    document
      .getElementById("send-direct-btn")
      .addEventListener("click", () => this.startDirectSend());

    // Send mode
    document.querySelectorAll('input[name="send-mode"]').forEach((radio) => {
      radio.addEventListener("change", (e) =>
        this.toggleSchedule(e.target.value),
      );
    });
    ...
    ["schedule-date", "schedule-hour", "schedule-minute"].forEach((id) => {
      document
        .getElementById(id)
        .addEventListener("change", () => this.updateScheduleInfo());
    });

    // Resume
    document
      .getElementById("resume-btn")
      .addEventListener("click", () => this.resumeSend());
    ...
    document
      ...
      .addEventListener("change", () => this.loadDashboard());
    document
  ...
  updateButtons() {
    ...
    const genReady =
      this.state.files.excel &&
      this.state.files.cv &&
      this.state.files.template &&
      this.state.files.other;
    ...
    if (directBtn) {
      const directReady =
        this.state.files.direct_excel &&
        this.state.files.direct_letter &&
        this.state.files.direct_pdf;
  ...
  // ==========================================
  async startGenerate() {
    const bewerbungsname =
      document.getElementById("bewerbungsname").value.trim() || "Bewerbung";
    const position =
      parseInt(document.getElementById("anschreiben-pos").value) || 2;
  ...
  async startImport() {
    ...
    try {
      ...
      if (data.success) {
        ...
        document.getElementById("gen-success").classList.remove("hidden");
        document.getElementById("gen-success-title").textContent =
          "Import erfolgreich!";
        this.toast(
          `${data.total_companies} Unternehmen importiert!`,
          "success",
        );
      } else {
    ...
    fetch("/api/send/status")
      ...
      .then((data) => {
        ...
        if (data.send_done) {
          ...
          document
            .getElementById("send-config-section")
        ...
        if (data.interrupted_at) {
          ...
          document
            ...
            .classList.add("hidden");
          document
            .getElementById("send-interrupted")
        ...
        if (data.sending || data.waiting_scheduled) {
          ...
          if (data.waiting_scheduled) {
            document
              .getElementById("schedule-countdown")
              .classList.remove("hidden");
        ...
        if (isDirect) {
          ...
          document
            .getElementById("send-direct-form")
            .classList.remove("hidden");
  ...
  async startSend() {
    ...
    const sendMode = document.querySelector(
      'input[name="send-mode"]:checked',
    ).value;
  ...
  async startDirectSend() {
    ...
    try {
      const resp = await fetch("/api/send/direct", {
        method: "POST",
        body: formData,
      });
      const data = await resp.json();
  ...
  async resumeSend() {
    const resumeFrom =
      parseInt(document.getElementById("resume-num").value) || 1;
  ...
  async resetAll() {
    try {
      ...
      document.getElementById("import-mode-section").classList.add("hidden");
      document
        .getElementById("generate-mode-section")
        .classList.remove("hidden");
      document.getElementById("send-direct-form").classList.add("hidden");
      document.getElementById("send-config-section").classList.add("hidden");
      ...
      document
        .querySelectorAll(".upload-filename")
  ...
  async loadDashboard() {
    ...
    try {
      const resp = await fetch(
        `/api/dashboard?session=${encodeURIComponent(session)}&hours=${hours}`,
      );
      const data = await resp.json();
      ...
      if (data.companies.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6" class="no-data">Keine Daten vorhanden</td></tr>';
      } else {
  ...
  async loadExcelFiles(session) {
    try {
      const resp = await fetch(
        `/api/dashboard/excels?session=${encodeURIComponent(session)}`,
      );
      const data = await resp.json();
      ...
      if (data.files.length === 0) {
        list.innerHTML =
          '<div class="no-data">Keine Excel-Dateien gespeichert.</div>';
        return;
      }
